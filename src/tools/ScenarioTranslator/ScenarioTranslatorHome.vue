<template>
  <n-card>
    ...
    <n-dropdown trigger="hover" :options="LanguageList" @select="changeLanguage">
      <n-button style="margin-right: 20px">{{ hm[mainStore.getLanguage] }}</n-button>
    </n-dropdown>
    <n-button @click="copyToClipboard('[\\]')" style="margin-right: 20px">复制标记</n-button>
  </n-card>
  <n-card>
    临时剧情翻译GUI<br />
    请注意原句中出现的"<span style="color: red">[\]</span>"标记是分割标记, 标记前后可能是一个完整的句子或者两个独立的句子<br />
    在翻译的时候请注意保持句子中[\]标记数量的一致性<br />
    如果对于标记有任何疑问,
    可以只输入翻译完成后的句子并在最前方加上"UnFormatted"供技术人员修改<br />
  </n-card>
  <TranslatorCard v-for="(line, index) in mainStore.getScenario.content" :index="index" :line="line"
    :language="mainStore.getLanguage" />
</template>
<script setup lang="ts">
import { saveAs } from 'file-saver';
import { NButton, NCard, NInput, NDropdown } from 'naive-ui';
import TranslatorCard from './components/TranslatorCard.vue';
import { useMainStore } from './store/mainStore';
import { Scenario, Language } from './types/content';
